...
{
    public abstract class Soliterax_Server
    {
        ...
        private void ClientReceiveCallBack(IAsyncResult ar)
        {
            ...
            if(socket.Connected)
            {
                ...
                if(received != 0)
                {
                    ...
                    ClientMessage(Encoding.ASCII.GetString(dataBuıffer));
                }
            } else
            {
                ControlLogOut(socket);
        ...
        /// <summary>
        /// if server whatever problem or application exit command login this veriable running.
        /// </summary>
        public abstract void ServerCloseEvent();

        /// <summary>
        /// if Client logout this method calling
        /// </summary>
        /// <param name="socket">Client datas veriable</param>
        /// <returns></returns>
        public abstract bool ControlLogOut(Socket socket);

        /// <summary>
        /// if Client login this method calling
        /// </summary>
        /// <param name="socket">Client datas veriable</param>
        public abstract void LoginEvent(Socket socket);

        /// <summary>
        /// if Client send a message this method calling
        /// </summary>
        /// <param name="message">Message veriable from Client</param>
        public abstract void ClientMessage(string message);
